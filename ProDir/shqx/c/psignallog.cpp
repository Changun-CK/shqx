/*
 * 本程序用于处理分区预警发布日志, 并保存到数据库的T_SIGNALLOG表中
 * 作者: 陈锟  日期: 2020-02-16
*/
#include "_ooci.h"
#include "_shqx.h"
...
int main(int argc, char *argv[])
{
	if (argc!=5)
	{
		printf("\n本程序用于处理分区预警信号发布日志, 保存到数据库的T_SIGNALLOG表中.\n");
		printf("\n用法: /ProDir/shqx/bin/psignallog 数据文件存放的目录 日志文件名 数据库连接参数 程序运行时间间隔\n");
		printf("\n例如: /ProDir/shqx/bin/psignallog /data/shqx/sdata/wpfiles /log/shqx/psignallog.log shqx/pwdidc@snorcl11g_31 10\n\n");
		return -1;
	}
	...
	{
		/* 扫描数据文件存放的目录 */
		if (Dir.OpenDir(argv[1], "WP20*.DTB", 1000, false, true) == false)
		{
...
bool _psignallog()
{
	...
	logfile.WriteEx("成功(总记录=%u, 插入=%u, 更新=%u, 丢弃=%u).\n", SIGNALLOG.m_total, SIGNALLOG.m_inserted, SIGNALLOG.m_updated, SIGNALLOG.m_discarded);
