...
int main(int argc, char *argv[])
{
	...
	// 本程序的业务流程主函数
	if (_quyujisuan() == false) { logfile.Write("total caculation tables failed.\n"); return -1; }
	logfile.Write("Calculation tables success.\n");
...
bool _quyujisuan()
{
	...
	// 计算滑动雨量
	if (quyujisuan.praind() != 0) return false;
	conn.commit();
	logfile.Write("表T_GDOBTRAIND处理完成.\n");
...
void Help(char *argv[])
{
	...
	printf("\nSample: /ProDir/shqx/bin/quyujisuan /log/shqx/quyujisuan.log "\
"\"<connstr>shqx/pwdidc@snorcl11g_31</connstr><charset>SIMPLIFIED CHINESE_CHINA.AL32UTF8</charset>"\
"<tname>T_AREAOBTMIND</tname><cols>to_char(ddatetime, 'yyyymmddhh24miss'), obtid, "\
"station, baoformat, bps, ys, w3a, wdidf, wdidd, wd2df, wd2dd, wd10df, wd10dd, "\
"wd3smaxdf, wd3smaxdd, wd3smaxtime, wd10maxdf, wd10maxdd, wd10maxtime, wd3daymax,"\
" wf3daymax, wd3daymaxtime, wd10daymax, wf10daymax, wd10daymaxtime, t, maxt,"\
" maxttime, mint, minttime, daymaxt, daymaxttime, daymint, dayminttime, rh, "\
"maxrh, maxrhtime, minrh, minrhtime, dp, p, maxp, maxptime, minp, minptime, p0"\
", hourrf, dayrf, rfmark1, minrf, wpv, othfields, rddatetime, procsts</cols>"\
"<fieldname>ddatetime, obtid, station, baoformat, bps, ys, w3a, wdidf, wdidd, "\
"wd2df, wd2dd, wd10df, wd10dd, wd3smaxdf, wd3smaxdd, wd3smaxtime, wd10maxdf, "\
"wd10maxdd, wd10maxtime, wd3daymax, wf3daymax, wd3daymaxtime, wd10daymax, "\
"wf10daymax, wd10daymaxtime, t, maxt, maxttime, mint, minttime, daymaxt, "\
"daymaxttime, daymint, dayminttime, rh, maxrh, maxrhtime, minrh, minrhtime, "\
"dp, p, maxp, maxptime, minp, minptime, p0, hourrf, dayrf, rfmark1, minrf, wpv,"\
" othfields, rddatetime, procsts</fieldname><andstr_hour>and procsts = 1 and "\
"substr(to_char(ddatetime, 'yyyy-mm-dd hh24:mi:ss'), 15, 5) = '00:00'"\
"</andstr_hour><andstr_real>and procsts = 1</andstr_real>\"\n\n");
	...
	printf("<andstr_real> and obtid in ('59293','50745')</andstr_real> 导出数据的附加条件, 注意, 关键字and不能缺.\n");
