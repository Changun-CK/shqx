...
int main(int argc,char *argv[])
{
  ...
  unsigned int uSaveCount=0;     // 归档日志存在的个数

  if (argc != 3)
  {
    printf("\n");
    printf("Using:/ProDir/public/bin/deletearchive pathname savecount\n\n");

    printf("Example:/ProDir/public/bin/deletearchive /oracle/archive1 5\n\n");
  ...
  while (1)
  {
    ...
    if (uFetchedCount < uSaveCount + 2 ) continue; // 文件个数小于uSaveCount不删除, 注意这里执行结果第一行是总用量, 所以个数+2
