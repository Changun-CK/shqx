...
void reverse(char *s)
{
	...
	for (ii = 0; ii < kk; ii++) {
	    temp = s[ii];
	    s[ii] = s[strlen(s) - 1 - ii];
	    s[strlen(s) - 1 - ii] = temp;
	}
}
...
int MaxChildStr(const char *str1, const char *str2, char *str3)
{
	 ...
	 *2.二维数组若有元素相等, 置为1, 不相等置为0, 连续相等的+1
	 *3.最大长度即为该数组元素的最大值
    ...
    for (int ii = 0; ii < len1; ii++)
    {
        for (int jj = 0; jj < len2; jj++)
        {
            if (str1[ii] == str2[jj])
            {
                if ( (ii == 0) || (jj == 0) ) arry[ii][jj] = 1;
                else arry[ii][jj] = 1 + arry[ii-1][jj-1];
            }
            else arry[ii][jj] = 0;

            if (arry[ii][jj] > maxlen) { maxlen = arry[ii][jj]; maxend = ii; }
        }
    }
